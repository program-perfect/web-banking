...
const transferStats = [
  { label: "2FA", value: "Passkey" },
  { label: "Limit", value: "$10k" },
  { label: "Fee", value: "$0–0.35" },
...
function DashboardView() {
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      <PixelCard
        className="payment-hero-card lg:col-span-2 pixel-gradient"
        eyebrow="Total balance"
        title={<PixelAmount value={total} className="text-3xl font-black" />}
        action={<PixelStatus tone="success">+6.8%</PixelStatus>}
      >
        <div className="grid gap-3 md:grid-cols-3">
          {accounts.map((account) => (
            <AccountTile key={account.id} title={account.name} detail={`${account.type} · ${account.number}`} amount={account.balance} />
          ))}
        </div>
        <div className="mt-5 grid grid-cols-2 gap-3">
          <NewTransferButton className="bg-primary text-primary-foreground" />
          <PixelButton><ArrowDownLeft className="h-4 w-4" />Request</PixelButton>
        </div>
      </PixelCard>
      ...
      <PixelCard title="Быстрые платежи" eyebrow="Contacts">
        <div className="flex -space-x-2">
          {contacts.map((contact) => (
            <Avatar key={contact.id} className="h-10 w-10 rounded-none border-2 border-foreground">
              <AvatarFallback className="rounded-none bg-secondary text-xs font-black text-secondary-foreground">{contact.initials}</AvatarFallback>
            </Avatar>
          ))}
        </div>
        <Link href={`/payments/${paymentHistory[0].id}`} className="mt-4 flex cursor-pointer items-center justify-between border-2 border-foreground bg-secondary p-3 hover:bg-card pixel-shadow-sm">
          <span>
            <span className="block text-sm font-black text-foreground">Последняя операция</span>
      ...
      <div className="grid gap-5 lg:col-span-3 md:grid-cols-3">
        ...
        <MetricCard icon={Wallet} label="BLOK wallet" value={formatUsd(walletTotal)} detail="Assets and staking" />
        <MetricCard icon={Receipt} label="Счета" value={formatUsd(upcomingBills)} detail="Следующие 30 дней" />
      </div>

      <PixelCard className="lg:col-span-3" title="Активность" action={<PixelStatus tone="info">Live ledger</PixelStatus>}>
    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_260px]">
          <div className="space-y-4">
            <p className="max-w-xl text-sm leading-relaxed text-foreground">
              Каждый перевод открывается как отдельная operation-page: выбор получателя, сумма, проверка, анимация обработки, салют и чек.
            </p>
            <div className="payment-stat-strip">
              {transferStats.map((stat) => <InfoPill key={stat.label} label={stat.label} value={stat.value} />)}
            </div>
          ...
          <div className="flex flex-col justify-between border-2 border-foreground bg-card p-4 pixel-shadow-sm">
            <div>
              <p className="font-pixel text-[9px] uppercase text-muted-foreground">New operation</p>
              <p className="mt-2 text-sm text-foreground">Создай черновик перевода с собственным slug и банковским чеком.</p>
            </div>
      ...
      <PixelCard title="Сводка" eyebrow="Today">
        <div className="space-y-3">
          ...
          <InfoLine icon={ShieldCheck} label="Risk checks" value="3/3" />
        </div>
      </PixelCard>
      ...
      <PixelCard title="Skeleton / loading" eyebrow="Preview">
        <div className="space-y-3">
...
function CardsView() {
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      <MetricCard icon={CreditCard} label="Доступно" value={formatUsd(available)} detail={`${cards.length} cards`} />
      <MetricCard icon={Lock} label="Заморожено" value={String(cards.filter((card) => card.frozen).length)} detail="Instant controls" />
      <MetricCard icon={ShieldCheck} label="3DS" value="ON" detail="Every card protected" />
            ...
            <div key={card.id} className="flex min-h-48 flex-col justify-between border-2 border-foreground bg-secondary p-5 pixel-shadow-sm">
              ...
              <div className="flex items-end justify-between gap-3">
                <span>
                  ...
                  <span className="block text-sm font-black text-foreground">{card.holder}</span>
                </span>
                <span className="text-right">
                  <span className="block text-[10px] uppercase tracking-wider text-muted-foreground">Balance</span>
                  <PixelAmount value={card.balance} className="text-sm font-black" />
                </span>
              </div>
            </div>
          ))}
...
function WalletView() {
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      <MetricCard icon={Wallet} label="Wallet value" value={formatUsd(totalAssets)} detail="BLOK assets" />
      <MetricCard icon={Zap} label="Staking APY" value="4.6%" detail="Daily rewards" />
      <MetricCard icon={ArrowUpRight} label="24h" value="+7.2%" detail="Portfolio" />
          ...
          {cryptoAssets.map((asset) => (
            <div key={asset.id} className="flex items-center justify-between gap-3 p-4">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center border-2 border-foreground bg-secondary font-pixel text-[9px]">{asset.symbol.slice(0, 3)}</span>
                <div>
              ...
              <div className="text-right">
                <PixelAmount value={asset.amount * asset.priceUsd} className="text-sm font-black" />
                <p className={cn("text-xs font-black", asset.change24h >= 0 ? "text-success" : "text-destructive")}>{asset.change24h >= 0 ? "+" : ""}{asset.change24h}%</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <PixelCard title="Actions">
        <div className="grid gap-3">
          {["Receive", "Send", "Swap", "Stake"].map((label) => <PixelButton key={label} className="w-full justify-between">{label}<ArrowUpRight className="h-4 w-4" /></PixelButton>)}
        </div>
      </PixelCard>
    </div>
  )
...
function SavingsView() {
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      <MetricCard icon={PiggyBank} label="Saved" value={formatUsd(savedTotal)} detail={`${Math.round((savedTotal / targetTotal) * 100)}% goals`} />
      <MetricCard icon={Target} label="Target" value={formatUsd(targetTotal)} detail="3 vaults" />
      <MetricCard icon={Landmark} label="Auto-save" value="$700" detail="Rules active" />
      <PixelCard className="lg:col-span-2" title="Vaults">
        <div className="space-y-4">
            ...
            return (
              <div key={vault.id} className="border-2 border-foreground bg-secondary p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-black text-foreground">{vault.name}</p>
                    <p className="text-xs text-muted-foreground">{vault.rule} · {vault.apy} APY</p>
                  </div>
                  <p className="text-sm font-black tabular-nums text-foreground">{formatUsd(vault.saved)} / {formatUsd(vault.target)}</p>
                </div>
                <div className="mt-4 h-3 border-2 border-foreground bg-card"><div className="h-full bg-primary" style={{ width: `${progress}%` }} /></div>
              </div>
            )
          })}
        </div>
      ...
      <PixelCard title="Smart rules">
        <div className="space-y-3">
          {["Round every purchase", "Move 12% of salary", "Boost every Friday"].map((rule) => <InfoLine key={rule} icon={BadgeCheck} label={rule} value="ON" />)}
        </div>
      </PixelCard>
    </div>
  )
...
function BillsView() {
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      ...
      <MetricCard icon={CalendarClock} label="Autopay" value="3" detail="No missed bills" />
      <MetricCard icon={FileText} label="Paid" value="$19.00" detail="This month" />
        ...
        <div className="divide-y-2 divide-border border-2 border-foreground bg-card">
          {bills.map((bill) => (
            <div key={bill.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center border-2 border-foreground bg-secondary"><Receipt className="h-4 w-4 text-primary" /></span>
                <div>
                  <p className="text-sm font-black text-foreground">{bill.name}</p>
                  <p className="text-xs text-muted-foreground">{bill.category} · {bill.due}</p>
                </div>
              </div>
              <div className="text-right">
                <PixelAmount value={bill.amount} className="text-sm font-black" />
                <PixelStatus tone={bill.status === "paid" ? "success" : bill.autopay ? "info" : "warning"}>{bill.status === "paid" ? "Paid" : bill.autopay ? "Autopay" : "Manual"}</PixelStatus>
              </div>
            </div>
          ))}
        </div>
...
function SupportView() {
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      <PixelCard className="lg:col-span-2" eyebrow="Help center" title="Как помочь?" action={<PixelStatus tone="success">4 min</PixelStatus>}>
        <div className="grid gap-3 sm:grid-cols-3">
          {[{ label: "Chat", icon: LifeBuoy }, { label: "Lock account", icon: Lock }, { label: "Report card", icon: CreditCard }].map(({ label, icon: Icon }) => (
        ...
        <div className="mt-6 divide-y-2 divide-border border-2 border-foreground bg-card">
          {supportTickets.map((ticket) => (
            <div key={ticket.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
              <div>
                <p className="text-sm font-black text-foreground">{ticket.title}</p>
                <p className="text-xs text-muted-foreground">{ticket.id} · {ticket.updated}</p>
              </div>
